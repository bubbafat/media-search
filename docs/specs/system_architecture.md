...
6. **The Garbage Collector Worker (Disk & DB Bound):**
   ...
   - **MaintenanceService:** A central `MaintenanceService` (`src/core/maintenance.py`) consolidates janitor tasks: prune stale workers (worker_status rows older than 24h), reclaim expired leases (assets stuck in `processing`), and cleanup `data_dir/tmp` files older than 4h. Temp cleanup is transcode-aware: it skips tmp when a local worker has `stats->>'current_stage' == 'transcode'` (seen in last 120s) to avoid deleting partial FFmpeg output. A future GC worker implementation can invoke `MaintenanceService.run_all()` in addition to trash-empty and orphan cleanup.
