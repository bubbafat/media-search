...
**Segmentation versioning:** `asset.segmentation_version` tracks the `PHASH_THRESHOLD` and `DEBOUNCE_SEC` values used when scene indexing completed. If these parameters change (e.g. in code), the Video Proxy Worker detects the mismatch, clears scene data for affected assets, resets them to `pending`, and re-segments on the next pass. Legacy assets with NULL `segmentation_version` are not invalidated. **FFmpeg exit check:** If FFmpeg exits non-zero during scene indexing, the run is rejected regardless of duration metadata (avoids marking partial/crashed decodes as proxied when probe_video_duration returns None, e.g. MKVs with missing duration headers). **Worker guard:** Before clearing scene data and resetting status for a stale-segmentation asset, the Video Proxy Worker verifies the source file exists via `resolve_path`. If the source is missing (ValueError, FileNotFoundError, or path does not exist), the worker skips that asset, logs a warning, and leaves its status unchanged. This protects assets whose source files are temporarily unavailable (e.g. unmounted volume) from destructive updates; permanent removal of such assets is handled by the maintenance task `reap_missing_source_files`.
...
6. **The Garbage Collector Worker (Disk & DB Bound):**
   ...
   - **MaintenanceService:** A central `MaintenanceService` (`src/core/maintenance.py`) consolidates janitor tasks: prune stale workers (worker_status rows older than 24h), reclaim expired leases (assets stuck in `processing`), cleanup `data_dir/tmp` files older than 4h, **purge deleted libraries** (wipe generated dirs on disk and hard-delete trashed libraries), and **reap assets with missing source files** (delete assets whose source files no longer exist on disk). Temp cleanup is transcode-aware: it skips tmp only when a local worker both reports `stats->>'current_stage' == 'transcode'` (seen in last 120s) *and* holds a valid unexpired lease on an asset; this avoids ghost workers (crashed workers whose status row lingers) from blocking tmp cleanup indefinitely. A future GC worker implementation can invoke `MaintenanceService.run_all()` in addition to trash-empty and orphan cleanup.
