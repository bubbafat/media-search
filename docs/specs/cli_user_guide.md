...
**Note:** This CLI command always uses PostgreSQL full-text search. The web dashboard search (`GET /api/search`) may use Quickwit when it is enabled in config (for All Media or any library filter, when indexes exist for that scope); the CLI does not call the web API.
