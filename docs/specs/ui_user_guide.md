...
**Requirements:** A running PostgreSQL instance and applied migrations (`alembic upgrade head`). The app reads from the same database as the CLI and workers. When Quickwit is enabled in config and at least one library has an active Quickwit index for the search scope (All Media or the selected library filter), search results may be served from Quickwit; otherwise (no indexes for that scope, or Quickwit unavailability) search uses PostgreSQL full-text.
